export function scrapeAmazonData() {
  ...
  if (productPriceElement) {
    productData.product.price = productPriceElement?.innerText?.split('\n')[0];
  }
  ...
  if (productRatingElement) {
    productData.product.platform_rating = productRatingElement?.innerText;
  }
  ...
  if (productReviewsCountElement) {
    productData.product.reviews_count =
      productReviewsCountElement?.innerText?.trim();
  }
  ...
  if (sellerNameElement) {
    ...
    productData.product.seller = {
      name: sellerNameElement?.innerText?.trim(),
  ...
  reviewElements.forEach((reviewElement) => {
    ...
    const platform_rating = reviewElement
      ...
      ?.innerText?.trim();
    const review_star = reviewElement
      .querySelector('.review-rating')
      ?.innerText?.split(' ')[0];
